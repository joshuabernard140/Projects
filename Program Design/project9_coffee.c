...
int compare(struct coffee *a, struct coffee *b);
...
int main(){
    ...
    qsort(list, count, sizeof(struct coffee), compare);
...
int compare(struct coffee *a, struct coffee *b) {
    ...
    struct coffee *copy_a = a;
    struct coffee *copy_b = b;

    if (copy_a->totalScore < copy_b->totalScore){
        return -1;
    }else if (copy_a->totalScore > copy_b->totalScore){
        return 1;
    }else{
        return 0;
    }
